 ...
 */
class iform_cocoast_easy_download_2 {
   ...
   */
  public static function get_cocoast_easy_download_2_definition() {
    return array(
      'title' => 'Cocoast : Easy download 2',
    ...
    foreach($terms as $term) {
    	...
    	if(count($result)>0) {
    		...
    		if(count($userIDList) > 0) {
    			$hubList[implode(',',$userIDList)] = $term->tid.' '.$term->name /* .' '.implode(',',$userIDList) */;
    		}
    	}
    }
    $r .= data_entry_helper::select(array(
    		'fieldname' => 'user_id_list',
    		'label' => lang::get('Hub to include'),
    		'helpText' => 'Choose a Hub, or &lt;All&gt; to not filter by Hub. This is driven off the users currently allocated to the hub. This currently does not apply to the NBN download.',
    		'lookupValues' => $hubList,
    		'class' => 'control-width-5'
    ));
    // End Cocoast Hub
  ...
  private static function build_params($args) {
    ...
    if (!empty($_POST['user_id_list']))
      $params['user_id_list']=$_POST['user_id_list'];
